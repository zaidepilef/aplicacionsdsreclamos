...
const DespachaDocumentacionElectronica = ({
  instanciaForm,
  handleSubmitForm
}) => {
  ...
  useEffect(() => {
    const getDigitalFolderApi = async () => {
      ...
      if (res.error) {
        setError(true);
        setLoading(false);
        return notificationsApi("error", "Error al cargar el expedient xdde");
      }
...
DespachaDocumentacionElectronica.propTypes = {
  instanciaForm: PropTypes.object.isRequired,
  handleSubmitForm: PropTypes.func.isRequired
};

export default DespachaDocumentacionElectronica;
