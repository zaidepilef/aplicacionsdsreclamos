...
const VistaPrevia = props => {
  ...
  const form = {
    ...
    solicitud: {
      fecha_creacion: {
        dia: formPdf.cotizante.hasOwnProperty("fecha_actual")
          ? formPdf.cotizante.fecha_actual[0].split("-")[2]
          : instanciaForm.fecha_inicio_reclamo.split("-")[2],
        mes: formPdf.cotizante.hasOwnProperty("fecha_actual")
          ? meses[parseInt(formPdf.cotizante.fecha_actual[0].split("-")[1]) - 1]
          : meses[parseInt(instanciaForm.fecha_inicio_reclamo.split("-")[1]) - 1],
        año: formPdf.cotizante.hasOwnProperty("fecha_actual")
          ? formPdf.cotizante.fecha_actual[0].split("-")[0]
          : instanciaForm.fecha_inicio_reclamo.split("-")[0]
      }
    },
    reclamo_previo: {
      nombre: formPdf.cotizante.hasOwnProperty("nombre_reclamante")
        ? formPdf.cotizante.nombre_reclamante[0]
        : "Bastian Manuel",
      apellido: formPdf.cotizante.hasOwnProperty("apellido_reclamante")
        ? formPdf.cotizante.apellido_reclamante[0]
        : "Gallardo",
      reclamo: formPdf.cotizante.hasOwnProperty("n_reclamo_anterior")
        ? formPdf.cotizante.n_reclamo_anterior[0]
        : "845587",
      fecha: {
        dia: formPdf.cotizante.hasOwnProperty("fecha_amterior")
          ? formPdf.cotizante.fecha_actual[0].split("-")[2]
          : arreglin.split("-")[2],
        mes: formPdf.cotizante.hasOwnProperty("fecha_amterior")
          ? meses[parseInt(formPdf.cotizante.fecha_actual[0].split("-")[1]) - 1]
          : meses[parseInt(arreglin.split("-")[1]) - 1],
        año: formPdf.cotizante.hasOwnProperty("fecha_amterior")
          ? formPdf.cotizante.fecha_actual[0].split("-")[0]
          : arreglin.split("-")[0]
      }
    }
